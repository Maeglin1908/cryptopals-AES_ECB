...
def bytesSub(data):
    res = b''.join([bytes([sbox[c]]) for c in data])
    return res


def bytesSubInv(data):
    return b''.join([bytes([sboxInv[c]]) for c in data])


def bitRotateLeft(b, c):
    return ((b << c) | (b >> 8 - c)) & 0xff


def byteRotateLeft(ar_b, c):
    return ar_b[c:] + ar_b[:c]


def splitSized(data, size):
    return [data[i * size:(i + 1) * size] for i in range(len(data) // size)]


def str_to_matrix(s):
    return transpose(splitSized(s, 4))


def transpose(ar):
    return [[ar[j][i] for j in range(len(ar))] for i in range(len(ar[0]))]


def calcRoundConstant(n):
    if n == 1:
        return 1
    else:
        res = calcRoundConstant(n-1)
        if res < 0x80:
            return 2*res
        else:
            return (2*res) ^ 0x11b
...
def applyRoundConstant(ar, round_num):
    round_constant = calcRoundConstant(round_num)
    return xorBytes(ar, [round_constant, 0, 0, 0])
...
def roundKeyGeneration(key, round_num):
    ...
    gw = bytesSub(gw)
    gw = applyRoundConstant(gw, round_num)
    w4 = xorBytes(w[0], gw)
    w5 = xorBytes(w4, w[1])
    w6 = xorBytes(w5, w[2])
    w7 = xorBytes(w6, w[3])
